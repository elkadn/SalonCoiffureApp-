...
export const checkUniqueCategoryName = async (nom, excludeId = null) => {
  try {
    ...
    // CORRECTION : Utiliser une requête exacte
    const q = query(
      categoriesRef,
      where("nom_lower", "==", nomLower), // Ajouter un champ en minuscule
      where("actif", "==", true)
    );
    
    const snapshot = await getDocs(q);
    
    // Filtrer par ID exclu
    const exists = snapshot.docs.some(doc => {
      return excludeId ? doc.id !== excludeId : true;
    });
    
    return !exists; // Retourne true si unique, false si existe
  } catch (error) {
    console.error("Erreur vérification nom catégorie:", error);
    // En cas d'erreur, on assume que c'est unique pour ne pas bloquer
    return true;
  }
};
