...
export const updateUser = async (id, userData) => {
  try {
    ...
    // Créez un objet avec seulement les données nécessaires
    const updateData = {};
    
    // Liste des champs autorisés
    const allowedFields = [
      'nom', 'prenom', 'telephone', 'role', 
      'adresse', 'dateNaissance', 'pointsFidelite'
    ];
    
    allowedFields.forEach(field => {
      if (userData[field] !== undefined) {
        // Conversion des types si nécessaire
        if (field === 'pointsFidelite') {
          updateData[field] = parseInt(userData[field]) || 0;
        } else {
    ...
    updateData.dateModification = serverTimestamp();
