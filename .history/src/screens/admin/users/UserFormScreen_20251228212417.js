...
const UserFormScreen = ({ navigation, route }) => {
  ...
  try {
    ...
    // MODIFICATION ICI : Passer un paramètre pour rafraîchir
    navigation.navigate('UserList', { refresh: true });
    // OU si tu veux retourner en arrière avec rafraîchissement
    // navigation.goBack();
    // Mais tu dois gérer le rafraîchissement dans UserListScreen
  } catch (error) {
    Alert.alert("Erreur", error.message || "Une erreur est survenue");
  } finally {
    setLoading(false);
