...
const ProfilCapillaireScreen = ({ navigation, route }) => {
  ...
  const openModal = (field, options, title, allowCustom = false) => {
    ...
    setModalOptions([
      ...options,
      ...(allowCustom ? [{ id: "custom", label: "➕ Autre..." }] : []),
    ]);
    setModalTitle(title);
  };
    ...
    return (
      <View style={styles.formGroup}>
        ...
        <TouchableOpacity
          ...
          onPress={() => openModal(field, options, label, allowCustom)}
        >
          <Text
  ...
  style={[
    styles.modalOption,
    formData[activeModal] === (option.id ? option.label : option) && styles.modalOptionSelected
  ]}
  onPress={() => selectOption(option.id ? option.label : option)}
>
  <Text style={[
    styles.modalOptionText,
    formData[activeModal] === (option.id ? option.label : option) && styles.modalOptionTextSelected,
    option === '➕ Autre...' && styles.customOptionText
  ]}>
    {option.id ? option.label : option}
  </Text>
  {formData[activeModal] === (option.id ? option.label : option) && (
