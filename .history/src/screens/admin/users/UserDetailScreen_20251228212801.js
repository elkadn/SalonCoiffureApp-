...
const UserDetailScreen = ({ navigation, route }) => {
  ...
  onPress={() => {
    if (userId === currentUserId) {
      Alert.alert("Information", "Vous ne pouvez pas modifier votre propre compte depuis cette interface");
    } else {
...
>
  <Icon
    ...
    style={styles.actionIcon}
  />
  <Text style={[styles.actionText, userId === currentUserId && { color: "#ccc" }]}>
    Modifier
  </Text>
</TouchableOpacity>

// Et aussi pour le bouton désactiver :
<TouchableOpacity
  style={[styles.actionButton, styles.deleteAction]}
  onPress={() => {
    if (userId === currentUserId) {
      Alert.alert("Information", "Vous ne pouvez pas désactiver votre propre compte");
    } else {
      handleDelete();
    }
  }}
