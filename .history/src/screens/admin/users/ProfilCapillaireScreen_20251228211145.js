...
} from '../../services/ProfileCapillaireClient';
            ...
            <View style={styles.modalContent}>
              ...
              <ScrollView>
                {options.map((option) => (
                  <TouchableOpacity
                    key={option}
                    style={[styles.modalOption, formData[field] === option && styles.modalOptionSelected]}
                    onPress={() => {
                      setFormData({...formData, [field]: option});
                      setModalVisible(false);
                    }}
                  >
                    <Text style={[styles.modalOptionText, formData[field] === option && styles.modalOptionTextSelected]}>
                      {option}
                    </Text>
                    {formData[field] === option && <Icon name="check" size={20} color="#4CAF50" />}
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
      ...
      <ScrollView style={styles.content}>
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            Renseignez le profil capillaire du client pour un suivi personnalisé des soins.
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {renderSelectField('Type de cheveux', 'typeCheveux', ['Normal', 'Sec', 'Gras', 'Mixte'])}
          {renderSelectField('État des cheveux', 'etatCheveux', ['Abimé', 'Normal', 'Sain'])}
          {renderSelectField('Texture des cheveux', 'textureCheveux', ['Raide', 'Ondulé', 'Bouclé', 'Frisé'])}
          {renderSelectField('Longueur des cheveux', 'longueurCheveux', ['Court', 'Moyen', 'Long'])}
          {renderTextField('Couleur naturelle', 'couleurNaturelle', 'Ex: Brun clair, Châtain...')}
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Santé et traitements</Text>
          {renderTextField('Allergies connues', 'allergies', 'Ex: Produits chimiques, Parfums...')}
          {renderTextField('Traitements actuels', 'traitementsActuels', 'Ex: Kératine, Coloration...')}
          {renderTextField('Produits utilisés', 'produitsUtilises', 'Shampoings, après-shampoings...')}
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Objectifs et notes</Text>
          {renderSelectField('Objectifs principaux', 'objectifs', ['Coupe', 'Coloration', 'Soin', 'Lissage', 'Autre'])}
          {renderTextField('Notes supplémentaires', 'notesSupplementaires', 'Autres informations importantes...', true)}
        </View>
...
// Styles similaires à StylisteCreneauxScreen
