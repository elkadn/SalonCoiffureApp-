...
const AffectSpecialites = ({ navigation, route }) => {
  // Ajoutez une vérification pour les params
  const params = route?.params || {};
  const coiffeurId = params.coiffeurId;
  const coiffeurName = params.coiffeurName || "Coiffeur";
  ...
  useEffect(() => {
    if (coiffeurId) {
      ...
      Alert.alert("Erreur", "Aucun coiffeur sélectionné");
      navigation.goBack();
    }
  }, [coiffeurId]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.coiffeurInfo}>
        ...
        <View style={styles.coiffeurDetails}>
          ...
          <Text style={styles.coiffeurId}>ID: {coiffeurId.substring(0, 8)}...</Text>
        </View>
