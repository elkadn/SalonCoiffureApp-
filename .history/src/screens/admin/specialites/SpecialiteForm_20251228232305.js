...
const SpecialiteForm = ({ navigation, route }) => {
  ...
  const loadSpecialite = async () => {
    try {
      ...
      Alert.alert("Erreur", "Impossible de charger la spécialité");
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  ...
  const validateForm = async () => {
    ...
    if (!nom.trim()) {
      ...
      try {
        const isUnique = await specialiteService.checkUniqueName(
          nom.trim(),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        await specialiteService.update(specialiteId, specialiteData);
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              ...
              value={nom}
              onChangeText={setNom}
              placeholder="Ex: Coupe Homme, Coloration, etc."
              maxLength={50}
              editable={!saving}
            />
            {errors.nom && <Text style={styles.errorText}>{errors.nom}</Text>}
            <Text style={styles.helperText}>
              Doit être unique parmi toutes les spécialités
            </Text>
          </View>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              Les spécialités permettent de catégoriser les compétences des
              coiffeurs et de mieux organiser les services offerts.
            </Text>
          </View>
        </View>
      </ScrollView>
