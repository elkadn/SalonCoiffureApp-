...
const ProductForm = ({ navigation, route }) => {
  ...
  // ProductForm.js - Version modifiée pour stockage local

// REMPLACEZ tout le code lié à Firebase Storage par ceci :
...
const handleSave = async () => {
  ...
  try {
    ...
    // Si une image a été sélectionnée, sauvegarder l'URI locale
    if (imageUri) {
      productData.imageUri = imageUri; // Stocker simplement l'URI
      console.log('Image URI sauvegardée:', imageUri);
    } else if (imageUrl) {
      productData.imageUri = imageUrl; // Garder l'ancienne si modification
    }
