...
import { pickImage, compressImage, uploadImage } from '../../../../services/imageService';
...
const ProductForm = ({ navigation, route }) => {
    ...
    try {
      ...
      if (isEditMode) {
        ...
        if (product) {
          ...
          const category = categoriesData.find(c => c.id === product.categorieId);
          const supplier = suppliersData.find(s => s.id === product.fournisseurId);
  ...
  const validateForm = async () => {
    ...
    if (!seuilAlerte || isNaN(parseInt(seuilAlerte)) || parseInt(seuilAlerte) < 0) {
  ...
  // Dans ProductForm.js, modifiez la fonction handleSave :
...
const handleSave = async () => {
  ...
  try {
    ...
    // Trouver la catégorie et le fournisseur sélectionnés
    ...
    // Uploader l'image si une nouvelle est sélectionnée
    if (imageUri) {
      try {
        const tempProductId = isEditMode ? productId : `temp_${Date.now()}`;
        finalImageUrl = await uploadImage(imageUri, tempProductId);
        
        if (!finalImageUrl) {
          Alert.alert(
            "Info", 
            "L'image n'a pas pu être uploadée, mais le produit sera créé sans image."
          );
        }
      } catch (uploadError) {
    ...
    // Ajouter l'URL de l'image seulement si elle existe
    if (finalImageUrl) {
      ...
      <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.formGroup}>
                ...
                <TouchableOpacity
                  style={[styles.selectInput, errors.supplier && styles.inputError]}
            ...
            <View style={styles.section}>
              ...
              <View style={styles.row}>
                ...
                <View style={[styles.formGroup, { flex: 1, marginRight: 10 }]}>
                  ...
                  <TextInput
                    style={[styles.input, errors.prixAchat && styles.inputError]}
                    value={prixAchat}
                ...
                <View style={[styles.formGroup, { flex: 1 }]}>
                  ...
                  <TextInput
                    style={[styles.input, errors.prixVente && styles.inputError]}
                    value={prixVente}
              ...
              <View style={styles.row}>
                ...
                <View style={[styles.formGroup, { flex: 1 }]}>
                  ...
                  <Text style={styles.helperText}>Alerte quand ≤ ce nombre</Text>
                </View>
              </View>
            </View>
          </View>
        ...
        >
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>Sélectionner un fournisseur</Text>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <Text style={[
                      styles.modalItemText,
                      selectedSupplier === item.id && styles.modalItemTextSelected
                    ]}>
                      {item.nom}
                    </Text>
                    <Text style={styles.modalItemDescription}>
