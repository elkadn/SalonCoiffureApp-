...
import { pickImage, compressImage } from '../../../../services/localImageService';
...
const ProductForm = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // SAUVEGARDE IMAGE LOCALE - VERSION SIMPLIFIÉE
if (imageUri) {
  try {
    // Utilisez l'API moderne de expo-file-system
    const productIdForFile = isEditMode ? productId : `temp_${Date.now()}`;
    const fileName = `product_${productIdForFile}.jpg`;
    const newPath = `${FileSystem.documentDirectory}${fileName}`;
    
    // Utilisez moveAsync au lieu de copyAsync
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.imageSection}>
              ...
              <TouchableOpacity
                ...
                disabled={saving}
              >
                // Dans la section image du formulaire :
