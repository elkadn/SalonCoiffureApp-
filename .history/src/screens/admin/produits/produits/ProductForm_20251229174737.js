...
import { saveImagePermanently } from "../../../../services/localImageService";
...
const ProductForm = ({ navigation, route }) => {
  ...
  onst [imageUri, setImageUri] = useState(null); // URI temporaire de l'image sélectionnée
  ...
  const handleImagePick = async () => {
    try {
      ...
      if (uri) {
        setImageUri(uri); // Supprimez compressImage temporairement
      }
    } catch (error) {
      Alert.alert(
        "Erreur",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // === SAUVEGARDE IMAGE PERMANENTE ===
      if (imageUri) {
        try {
          console.log("Tentative de sauvegarde image permanente...");

          // Générer un ID temporaire si nouveau produit
          const tempProductId = isEditMode ? productId : `temp_${Date.now()}`;
          ...
          // Stocker le chemin permanent dans Firestore
          productData.localImagePath = permanentPath;
          console.log("✅ Image sauvegardée permanentement:", permanentPath);
        } catch (imageError) {
          console.error("❌ Erreur sauvegarde image permanente:", imageError);

          // Fallback: stocker l'URI temporaire avec un warning
          productData.imageUri = imageUri;
          Alert.alert(
            "Attention",
            "L'image n'a pas pu être sauvegardée de manière permanente, " +
              "elle pourrait disparaître plus tard. Le produit a quand même été créé."
          );
        }
      } else if (imageUrl) {
      ...
      if (isEditMode) {
        ...
        // Si c'est un nouveau produit et qu'on a utilisé un ID temporaire
        // Mettre à jour le chemin de l'image avec le vrai ID
        if (
          newProduct.id &&
          productData.localImagePath &&
          productData.localImagePath.includes("temp_")
        ) {
          const oldPath = productData.localImagePath;
          const newPath = oldPath.replace(/temp_\d+/, newProduct.id);
          ...
          try {
            ...
            console.log("✅ Image renommée avec vrai ID:", newPath);
          } catch (renameError) {
            console.error("Erreur renommage image:", renameError);
          }
        }
          ...
          <View style={styles.formCard}>
            ...
            <View style={styles.imageSection}>
              ...
              <TouchableOpacity
                ...
                disabled={saving}
              >
                {imageUri || (imageUrl && imageUrl.startsWith("file://")) ? (
                  <Image
