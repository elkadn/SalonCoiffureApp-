...
import { createNativeStackNavigator } from "@react-navigation/native-stack"; // CHANGÉ
...
export const CustomDrawer = ({ navigation }) => {
  ...
  const { currentUser: user, refreshUser } = useAuth(); // CHANGÉ: currentUser
  ...
  const menuItems = [
    ...
    ...(user?.role === "admin"
      ...
      : []),
    ...(user ? [{ label: "Déconnexion", action: () => {
        // À implémenter: fonction de déconnexion Firebase
